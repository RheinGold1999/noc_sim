...
crossbar_env::build_phase(uvm_phase& phase)
{
  ...
  for (int i = 0; i < num_mst; ++i) {
    ...
    if (!uvm_config_db<crossbar_if*>::get(this, mst_name, "vif", master_vifs[i])) {
      UVM_FATAL("NO_VIF", "vif must be set for: "
        + get_full_name() + "." + mst_name + ".vif");
    }
  ...
  for (int i = 0; i < num_slv; ++i) {
    ...
    if (!uvm_config_db<crossbar_if*>::get(this, slv_name, "vif", slave_vifs[i])) {
      UVM_FATAL("NO_VIF", "vif must be set for: "
        + get_full_name() + "." + slv_name + ".vif");
    }
